...
export default function useTimelineActions({
 ...
 // dentro do hook useTimelineActions
const setCellComment = async (rowId, dayISO, comment, authorInitial) => {
  ...
  try {
    ...
    // tente achar o comment retornado pelo back
    ...
    if (created && created.id != null) {
      const norm = {
        ...
        authorInitial:
          (created.author_initial || created.authorInitial || authorInitial || "U").toUpperCase(),
      ...
      setRows((prev) =>
        prev.map((r) => {
          ...
          return { ...r, cells: { ...r.cells, [dayISO]: { ...prevCell, comments: nextComments } } };
        })
      );

      return norm; // <=== devolve para a paleta usar sem "tmp-"
    } else {
      // sem payload do back: re-sincroniza e não retorna nada
  ...
  const toggleBaseline = async (rowId, dayISO, enabled) => {
    ...
    try {
      ...
      setRows((prev) => prev.map((r) => {
        ...
        const prevCell = r.cells?.[dayISO];
        const base = typeof prevCell === "object" ? prevCell : {};
        return { ...r, cells: { ...r.cells, [dayISO]: { ...base, baseline: !!enabled } } };
      }));
    } catch (e) {
      if (!handleTimelineError(e)) await fetchProjetoIfNeeded();
    }
  ...
  const deleteComment = async (rowId, dayISO, commentId) => {
    try {
      await apiLocal.deleteProjetoCellComment(projectId, rowId, dayISO, commentId, Number(actorSectorId));
      setRows((prev) => prev.map((r) => {
        if (r.id !== rowId) return r;
        const prevCell = r.cells?.[dayISO] || {};
        const prevComments = Array.isArray(prevCell.comments) ? prevCell.comments : [];
      ...
      if (status === 403) toast.error("Sem permissão para apagar este comentário.");
      else if (status === 423) toast.error("Projeto bloqueado. Ação não permitida.");
  ...
  const renameRow = async (rowId, newTitle) => {
    ...
    const snapshot = rows;
    setRows((prev) => prev.map((r) => (r.id === rowId ? { ...r, titulo: newTitle } : r)));
    try {
  ...
  const setRowSector = async (rowId, sectorKey) => {
    ...
    const next = current.includes(sectorKey) ? current.filter((k) => k !== sectorKey) : current.concat([sectorKey]);
  ...
  const addRow = async ({ titulo, sectorKeys }) => {
    await apiLocal.addProjetoRow(projectId, { title: titulo, row_sectors: sectorKeys || [] }, Number(actorSectorId));
    await fetchProjetoIfNeeded();
  };
