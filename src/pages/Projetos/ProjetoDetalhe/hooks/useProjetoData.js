...
const actorSectorId = useMemo(() => {
  ...
  const idsToKeys = (arr) => Array.isArray(arr) ? arr.map((id) => keyFromId(id)).filter(Boolean) : [];
  ...
  const fetchProjeto = useCallback(async () => {
    ...
    try {
      ...
      const fixed = rawRows.map((row) => {
        ...
        for (let i = 0; i < arr.length; i++) { if (typeof arr[i] === "number") { containsNumber = true; break; } }
        return containsNumber ? { ...row, sectors: idsToKeys(arr) } : row;
      });
      setRows(fixed);
  ...
  const projectSectorKeys = useMemo(() => idsToKeys(projeto?.setores || []), [projeto?.setores, sectors]);
  ...
  const handleTimelineError = (err) => {
    ...
    if (status === 423) { toast.warning("Projeto bloqueado: não é possível criar/editar tarefas."); return true; }
    if (status === 404) { toast.error("Atividade/célula não encontrada."); return true; }
    if (status === 403) { toast.error("Sem permissão para executar esta ação."); return true; }
    if (status >= 500) { toast.error(err?.response?.data?.detail || "Erro no servidor ao atualizar a timeline."); return true; }
    return false;
  };
  ...
  const toggleProjectLock = async () => {
    ...
    await loading.wrap("lock", async () => {
      try {
        ...
        if (e?.response?.status === 403) { toast.error("Sem permissão para alterar o lock deste projeto."); return; }
        throw e;
      }
    });
  };
...
const changeStatus = async (status) => {
  ...
  const fetchProjetoIfNeeded = async () => { await fetchProjeto(); };
