// src/pages/Projetos/ProjetoDetalhe/Timeline/SectorDropdown.jsx
...
/* ---------- COMPONENTE ---------- */
export function SectorDropdown({ id, value, onChange, options, themeMode = "light" }) {
  ...
  const onListKeyDown = (e) => {
    ...
    if (e.key === "Enter")     { e.preventDefault(); const opt = options[highlight]; if (opt) handleSelect(opt.key); }
  };
  ...
  return (
    ...
    >
      <Combo>
        ...
        {open && (
          <ComboList ref={listRef} role="listbox" tabIndex={-1} onKeyDown={onListKeyDown}>
            {options.map((opt, i) => (
              <ComboItem
                key={opt.key}
                role="option"
