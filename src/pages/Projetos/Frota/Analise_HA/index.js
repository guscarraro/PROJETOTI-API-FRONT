...
export default function Analise_HA() {
  ...
  const users = useMemo(() => {
    ...
    for (let i = 0; i < arr.length; i++) {
      ...
      Array.from(daySet).forEach((d) => {
        let dayObj = null;
        for (let z = 0; z < byDay.length; z++) {
          if (String(byDay[z]?.data) === String(d)) {
            dayObj = byDay[z];
            break;
          }
        }
  ...
  function openInconsistencias(usuario_key, dayISO) {
    ...
    for (let i = 0; i < users.length; i++) {
      if (users[i]?.usuario_key === usuario_key) {
        u = users[i];
        break;
      }
    }
  ...
  function openPossiveisFaltas(usuario_key, dayISO) {
    ...
    const onlyDay = [];
    for (let i = 0; i < u.miss_items.length; i++) {
      if (String(u.miss_items[i]?.data) === String(dayISO)) onlyDay.push(u.miss_items[i]);
    }
    ...
    <>
      ...
      <RightRow>
        <div style={{ fontSize: 12, opacity: 0.9, maxWidth: 860 }}>
          ...
          <strong>&gt; {maxOk}</strong>. Se status for <strong>Ajustado</strong>, some da divergência.
        </div>
      </RightRow>
        ...
        <div style={{ width: "100%", height: 260 }}>
          <ResponsiveContainer>
            ...
            >
              ...
              />
              <Legend />
              <Bar dataKey="avg" name="Média (min)" fill={COLOR_AVG} radius={[6, 6, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </ChartCard>
      ...
      <ChartCard>
        ...
        <UserCardsGrid style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: 12 }}>
          ...
          {users.length === 0 && <div style={{ opacity: 0.75, padding: 12 }}>Sem dados no período selecionado.</div>}
        </UserCardsGrid>
      </ChartCard>
