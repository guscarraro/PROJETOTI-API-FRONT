...
export default function ModalAlmocoPendentes({ open, payload, onClose }) {
    ...
    <ModalBackdrop>
      <ModalCard>
        <TwoCols>
          <div>
            <ModalTitle>
              {title} {payload?.dayISO ? `• Dia ${formatXAxisDate(payload.dayISO)}` : `• Período`}
            </ModalTitle>
              ...
              {!loading && items.length === 0 && (
                <TRow>
                  <TCell colSpan={5} style={{ opacity: 0.8 }}>
                    Sem pendências para este filtro.
                  </TCell>
                </TRow>
              )}
            </tbody>
