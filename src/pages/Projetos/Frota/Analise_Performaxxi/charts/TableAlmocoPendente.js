...
function normalizeRotaNumber(rota) {
  ...
  if (!s || s === "-") return "-";
  // pega o primeiro número "grande" que aparecer, senão devolve o texto
  const m = s.match(/\d{3,}/);
  return m ? m[0] : s;
}
...
function speakOrBeep(msg, opts, lastSpeakAtRef) {
  const options = opts || {};
  const cooldownMs = Number(options.cooldownMs ?? 15000);
  const now = Date.now();

  if (lastSpeakAtRef && now - Number(lastSpeakAtRef.current || 0) < cooldownMs) return;
...
export default function TableAlmocoPendente({
  ...
  // ⚠️ IMPORTANTE:
  // - "tom vermelho" (⚠ + bg vermelho) deve ser APENAS última entrega
  // - então: criticalWhenFaltandoLE deve ficar 0 (ou o valor que represente "última")
  criticalWhenFaltandoLE = 0,

  // ✅ ainda usamos esse limiar separado pra "listar"
  // (se você estiver trazendo 13/15, 14/15, 15/15, o threshold do endpoint é 2,
  // mas o vermelho continua só no faltando<=0)
  ...
  const view = useMemo(() => {
    ...
    for (let i = 0; i < safeItems.length; i++) {
      ...
      // ✅ vermelho só na última entrega
      const isCritical = Number.isFinite(falt) && falt <= Number(criticalWhenFaltandoLE);
    ...
    // mantém ordenado: crítico em cima, depois por faltando (menor primeiro), depois motorista
    out.sort((a, b) => {
    ...
    // opcional: se quiser cortar a lista só nos "no limite", faça filtro aqui.
    // mas como seu endpoint já filtra, deixo aberto:
    // const filtered = [];
    // for (let i = 0; i < out.length; i++) {
    //   const falt = Number.isFinite(out[i].faltando) ? out[i].faltando : 999999;
    //   if (falt <= Number(listWhenFaltandoLE)) filtered.push(out[i]);
    // }
    // return { rows: filtered, total: filtered.length, critical };

    return { rows: out, total: out.length, critical };
  }, [safeItems, criticalWhenFaltandoLE]);

  useEffect(() => {
    const cur = Number(view.critical || 0);

    // fala apenas com crítico (última entrega), como você pediu
    if (announceToken > 0 && cur > 0) {
      speakOrBeep(
        `Atenção. Você possui ${cur} rota${cur === 1 ? "" : "s"} na última entrega sem lançar o almoço.`,
        { cooldownMs: 0 },
        lastSpeakAtRef
      );
    ...
    prevCriticalRef.current = cur;
  }, [view.critical, announceToken]);

  const panelBg = `linear-gradient(180deg, ${rgba("#f97316", 0.18)}, ${rgba("#f97316", 0.06)})`;
    ...
    >
      ...
      <div
        style={{
          ...
          animation: pulse ? "almocoPulse 1.6s ease-in-out infinite" : undefined,
        }}
      >
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          {/* ✅ vermelho só quando é ÚLTIMA ENTREGA */}
          {view.critical > 0 && (
            <span
              style={{
                display: "inline-flex",
      ...
      <div style={{ height: "calc(100% - 54px)", overflow: "auto", marginTop: 8, paddingRight: 4 }}>
        {view.total === 0 ? (
          <div style={{ fontSize: 12, opacity: 0.85, color: theme.text, padding: 8 }}>
            Nenhuma rota no limite.
          </div>
        ) : (
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
            <thead>
              <tr style={{ textAlign: "left", opacity: 0.9, color: theme.muted }}>
                <th style={{ padding: "8px 6px", width: 54 }}>Dia</th>
                <th style={{ padding: "8px 6px" }}>Motorista</th>
                <th style={{ padding: "8px 6px", width: 64 }}>Rota</th>
                <th style={{ padding: "8px 6px", textAlign: "right", width: 74 }}>Prog.</th>
              </tr>
            </thead>
            <tbody>
              {view.rows.map((r) => (
                <tr
                  ...
                  title={`${formatShortDate(r.data)} • ${r.motoristaFull} • Rota: ${r.rotaRaw}`}
                  style={{
                    borderTop: `1px solid ${rgba(theme.border, 0.9)}`,
                    cursor: onRowClick ? "pointer" : "default",

                    // ✅ vermelho somente última entrega
                    background: r.isCritical ? rgba("#ef4444", 0.10) : "transparent",
                  }}
                >
                  <td style={{ padding: "8px 6px", color: theme.text, fontWeight: 900 }}>
                    {formatShortDate(r.data)}
                  </td>

                  <td style={{ padding: "8px 6px", color: theme.text, fontWeight: r.isCritical ? 1000 : 800 }}>
                    {/* ✅ ícone vermelho só última entrega */}
                    {r.isCritical ? <span style={{ color: "#ef4444", marginRight: 6 }}>⚠</span> : null}
                    {r.motoristaShort}
                  </td>
                  ...
                  <td style={{ padding: "8px 6px", textAlign: "right", fontWeight: 1000, color: theme.text }}>
                    {r.progresso_txt}
                  </td>
                </tr>
              ))}
