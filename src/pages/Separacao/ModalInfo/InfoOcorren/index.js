...
import { DisabledOverlay } from "./style";

export default function InfoOcorren({
  conferencia,
  ocorrencias,
  isLoading = false,
  conferenciaConcluida,
}) {
  const evidences = Array.isArray(conferencia?.evidences)
    ? conferencia.evidences
    : [];
  ...
  const elapsed = Number(
    conferencia?.elapsed_seconds || conferencia?.elapsedSeconds || 0
  );
      ...
      {isLoading && (
        <LoadingOverlay>
          <div className="loader-box">
            ...
            <span>Carregando dados da conferência...</span>
          </div>
        </LoadingOverlay>
      )}

      <CardInner $blur={isLoading}>
        <CardHeader>Informações da conferência</CardHeader>

        <MetaWrap>
          <MetaPill>
            <strong>Conferente:</strong> {confBy}
          </MetaPill>
          <MetaPill>
            <strong>Duração:</strong> {fmtTime(elapsed)}
          </MetaPill>
          <MetaPill>
            <strong>Data/Hora:</strong> {confAt}
          </MetaPill>
        </MetaWrap>
          ...
          <ModalBody style={{ padding: 0 }}>
            <PreviewWrap onClick={closePreview}>
              <div className="toolbar" onClick={(e) => e.stopPropagation()}>
                <span>
                  {evidences.length > 0
                    ? `Imagem ${previewIdx + 1} de ${evidences.length}`
                    : "—"}
                </span>
              ...
              <div className="stage" onClick={(e) => e.stopPropagation()}>
                {evidences[previewIdx] ? (
                  <img
                    src={evidences[previewIdx]}
                    alt={`Evidência ${previewIdx + 1}`}
                  />
                ) : null}

                {evidences.length > 1 && (
                  <>
                    <NavBtnLeft onClick={prevImg} title="Anterior (←)">
                      ‹
                    </NavBtnLeft>
                    <NavBtnRight onClick={nextImg} title="Próxima (→)">
                      ›
                    </NavBtnRight>
                  </>
                )}
              ...
              <tbody>
                {ocorrencias.map((o) => (
                  <tr key={o.id}>
                    ...
                    <td style={{ whiteSpace: "nowrap" }}>
                      {o.quantidade != null ? Number(o.quantidade) : "—"}
                    </td>
                    <td
...
const CardWrap = styled.div`
  ...
  display: grid;
  gap: 10px;
  color: #312e81; /* indigo-900-ish */

  [data-theme="dark"] & {
    background: #1e1b4b; /* indigo/purple bem escuro */
    border-color: rgba(196, 181, 253, 0.7); /* purple-300 */
    color: #e5e7eb;
  }
`;
...
const Shot = styled.div`
  ...
  border-radius: 12px;
  overflow: hidden;
  border: 1px solid ${(p) => (p.$selected ? "#a855f7" : "#e5e7eb")};
  background: #ffffff;
  box-shadow: ${(p) =>
    p.$selected ? "0 0 0 2px rgba(168,85,247,.25) inset" : "none"};
  cursor: zoom-in;

  [data-theme="dark"] & {
    border-color: ${(p) =>
      p.$selected ? "rgba(196,181,253,.8)" : "rgba(255,255,255,0.08)"};
    background: #020617;
  }
