...
{data.length > 0 && (
  <p style={{ marginBottom: '4px', fontSize: '14px', fontWeight: 500 }}>
    {Math.round((calculateTotalNotesByStatus(data) / getTotalNotesRenderizadas()) * 100)}%
  </p>
)}
      <ProgressBar
        progress={
          data.length > 0
            ? (calculateTotalNotesByStatus(data) / getTotalNotesRenderizadas()) * 100
            : 0
        }
      />
          ...
          {item.notas.map((nf, noteIdx) => {
            ...
            return (
              <li
                key={noteIdx}
                style={{
                  background: isAgendada && ehForaSJP
